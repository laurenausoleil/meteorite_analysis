...
na_meteors %>% 
  ...
  print()
```
This shows us that the main time periods for missing values are the 1900s and the 2000s.

## Number of NAs by decade
```{r}
# Analyse NAs by time period from 1900 onwards
...
```
Now we can see that there is a spike in missing values for mass which peaks in the period 1990-2000 and a growing pattern of missing values for location which starts in the mid 20th century and increases rapidly from 1990 until 2010.
The number of missing locations is at its highest, 6300 in the years 2000-2010.
...
This shows that the vast majority of missing values are for meteors which have been found rather than recorded while falling.
...
To get a rough idea of this, I would group meteor findings by latitude.
...
Number of meteor findings by location
```{r}
